...
export function printDetailedAnalysis(result: Result996, parsedData: ParsedGitData): void {
  console.log(chalk.cyan.bold('📋 詳細分析:'))
  console.log()

  const analysis: string[] = []

  // 1. 加班強度分析（與 calculator 的描述保持一致）
  ...
  if (parsedData.lateNightAnalysis) {
    ...
    if (totalLateNight > 20) {
      analysis.push(`🌙 深夜加班頻繁（${totalLateNight}天），嚴重影響健康`)
    } else if (totalLateNight > 10) {
      analysis.push(`🌃 存在深夜加班情況（${totalLateNight}天），需注意休息`)
    } else if (totalLateNight > 0) {
      analysis.push(`💡 偶有深夜加班（${totalLateNight}天），整體可控`)
    }
