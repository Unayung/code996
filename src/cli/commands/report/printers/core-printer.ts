...
/**
 * 核心結果打印器
 * 負責打印996指數、加班比例、總提交數等核心指標
 */

/** 打印核心指標（整合統計資訊，統一表格展示） */
export function printCoreResults(
  ...
  console.log(chalk.cyan.bold('📊 核心結果:'))
  console.log()
  ...
  resultTable.push(
    [
      { content: chalk.bold('996指數'), colSpan: 1 },
      { content: indexColor(result.index996.toFixed(1)), colSpan: 1 },
    ],
    [
      { content: chalk.bold('整體評价'), colSpan: 1 },
      { content: result.index996Str, colSpan: 1 },
    ],
    [
      { content: chalk.bold('分析時段'), colSpan: 1 },
    ...
    [
      { content: chalk.bold('總提交數'), colSpan: 1 },
  ...
  // 在核心結果表格下方添加996指數說明
  console.log(chalk.gray('* 996指數：為 0 則不加班，值越大代表加班越嚴重，996 工作制對應的值為 100。'))
  console.log()
}
